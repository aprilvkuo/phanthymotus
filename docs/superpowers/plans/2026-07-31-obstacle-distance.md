...
- [x] **Step 1:** 写测试，覆盖 PNG/JPEG 场景识别、640×480 ROI 映射、NaN/零值过滤、P1 与检测框裁剪。
- [x] **Step 2:** 运行 `uv run --python 3.12 --with pytest --with numpy pytest perception/tests/test_obstacle_distance_postprocess.py -q`，确认因模块缺失而失败。
- [x] **Step 3:** 实现不可变数据类型和纯 NumPy 后处理；坐标统一使用左闭右开区间，并用 `np.percentile(values, 1)`。
- [x] **Step 4:** 重跑测试并确认通过。
...
- [x] **Step 1:** 写测试，使用本地 `file://` URL 验证首次下载、缓存命中、SHA256 不匹配清理和不完整临时文件不覆盖旧模型。
- [x] **Step 2:** 运行目标测试，确认正确失败。
- [x] **Step 3:** 使用 `urllib.request`、同目录 `.part` 文件、`os.replace` 和分块 SHA256 实现下载器；实现懒加载 Transformers/Ultralytics 后端。
- [x] **Step 4:** 重跑目标测试并确认通过。
...
- [x] **Step 1:** 用 fake depth/detector 写测试，覆盖室内、无人车、无人车无检测目标回退、异常值夹紧和后端异常的保守结果。
- [x] **Step 2:** 运行目标测试并确认失败。
- [x] **Step 3:** 实现场景路由、焦距元数据、距离边界 `[0.05, 80.0]`、无人车检测为空时中心通行区回退，以及异常时 `0.5 m` 的显式降级结果。
- [x] **Step 4:** 重跑测试并确认通过。
...
- [x] **Step 1:** 写测试，注入 fake estimator，验证 stdout 无日志、坏路径退出码非零。
- [x] **Step 2:** 运行目标测试并确认失败。
- [x] **Step 3:** 实现库函数和 CLI；日志只写 stderr。
- [x] **Step 4:** 重跑测试并确认通过。
...
- [x] **Step 1:** 在不安装 ROS2 的情况下，通过 stub 模块测试 TOOLS schema、插件 dispatch 和输出 topic 命名。
- [x] **Step 2:** 运行目标测试并确认失败。
- [x] **Step 3:** 复用 VOP 的低延迟 QoS、单帧队列和 worker 模式实现插件，并在 `PerceptionBundle` 中按配置注册。
- [x] **Step 4:** 重跑测试并确认通过。
...
- [x] **Step 1:** 写文本级打包测试，验证 Dockerfile 复制插件、依赖存在、service.yml 挂载 `/models`、仓库无大模型后缀。
- [x] **Step 2:** 运行目标测试并确认失败。
- [x] **Step 3:** 增加固定版本依赖、环境变量和使用文档；明确默认只缓存模型，不写入 Git。
- [x] **Step 4:** 重跑测试并确认通过。
...
- [x] **Step 1:** 运行 `uv run --python 3.12 --with pytest --with numpy --with pillow pytest perception/tests -q`。
- [x] **Step 2:** 运行 Python 3.12 `compileall`、`git diff --check` 和模型文件大小扫描。
- [x] **Step 3:** 运行不下载权重的 CLI import smoke test。
- [x] **Step 4:** 检查参数预算、失败路径、文档命令与实际接口一致，并记录无法在本机完成的 Jetson/TensorRT 验证项。
